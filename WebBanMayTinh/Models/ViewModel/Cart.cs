...
namespace WebBanMayTinh.Models.ViewModel
{
    public class Cart
    {
        ...
        //Thêm vào giỏ hàng
        public void AddItems(int productId,string productName, string ProductImagine, decimal unitPrice, int quantity, string category, decimal productPrice, decimal? productDiscountValue)
        {
            ...
            {
                decimal discountValue = productDiscountValue ?? 0;


                CartProduct.Add(new CartItem { ProductId = productId,ProductName = productName, ProductImage = ProductImagine, UnitPrice = unitPrice, Quantity = quantity, productPrice = productPrice,ProductPriceDiscount = discountValue});
            }
        }
